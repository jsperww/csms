...
import com.hbsoft.csms.dao.NoticeBeanDao;
import com.hbsoft.csms.bean.NoticeBean;
import org.springframework.web.util.HtmlUtils;

@Service
public class NoticeBeanDaoService {
	@Autowired
	...
	public Map<String, Object> getPagingData(NoticeBean noticeBean) throws Exception {
		...
		for(NoticeBean nb:list){
			nb.setTitle(HtmlUtils.htmlUnescape(nb.getTitle()));
			nb.setContent(HtmlUtils.htmlUnescape(nb.getContent()));
			nb.setIssueTimeStr(DateUtil.formatDate(nb.getIssueTime(),"yyyy-MM-dd"));
		}
		map.put("code", 0);
		map.put("msg", "获取成功");
		map.put("count", count);
